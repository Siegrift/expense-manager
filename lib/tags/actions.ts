...
export const removeTag = (tagId: string): Thunk => async (
  dispatch,
  getState,
  { logger },
) => {
  ...
  if (txsWithTag.length !== 0) {
    dispatch(
      setSnackbarNotification(
        createErrorNotification(
          'Unable to remove the tag, because it is used in some transactions',
        ),
      ),
    )
  } else {
    ...
    dispatch(
      setSnackbarNotification(
        createSuccessNotification('Tag successfully removed'),
      ),
    )
  }
}
