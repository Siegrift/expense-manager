...
type Deserializers = {
  [key in keyof SerializableState]: Partial<
    Deserializer<SerializableState[key]>
  >
}
 ...
 */
export const csvFromDataSel = createSelector(
  serializableStateSel,
  ({ transactions, tags }) => {
...
export const jsonFromDataSel = createSelector(
  serializableStateSel,
  (serializableState) => JSON.stringify(serializableState, null, 2),
)

export const dataFromImportedJsonSel = (importedJsonString: string) =>
  createSelector(
    serializableStateSel,
    (serializableState): ImportedData => {
      let importedData: SerializableState = {
        tags: {},
        transactions: {},
        profile: {},
      }
      const keys = Object.keys(serializableState) as Array<
        keyof SerializableState
      >
      ...
      keys.forEach((key) => {
        ...
        desKeys.forEach((desKey) => {
          ...
          dataKeys.forEach((dataKey) => {
            importedData[key][dataKey][desKey] = desFn(
              importedData[key][dataKey][desKey],
            )
          })
        })
      })
